...
class WorkItemProvider(Provider):
    ...
    串联 MetadataService 和 WorkItemAPI，提供人性化的接口
    """
    def __init__(self, project_name: Optional[str] = None, project_key: Optional[str] = None):
        ...
        self.meta = MetadataService.get_instance()
    ...
    async def _resolve_field_value(self, project_key: str, type_key: str, field_key: str, value: Any) -> Any:
        """解析字段值：如果是 Select 类型且值为 Label，转换为 Option Value"""
        try:
            val = await self.meta.get_option_value(project_key, type_key, field_key, str(value))
            logger.info(f"Resolved option '{value}' -> '{val}' for field '{field_key}'")
            return val
        except Exception as e:
            logger.warning(f"Failed to resolve option '{value}' for field '{field_key}': {e}")
    ...
    async def create_issue(
        ...
        type_key = await self.meta.get_type_key(project_key, "Issue管理")
        ...
        # Description
        if description:
            field_key = await self.meta.get_field_key(project_key, type_key, "description")
        ...
        if priority:
            try:
                ...
                option_val = await self._resolve_field_value(project_key, type_key, field_key, priority)
                
                logger.info(f"Updating priority to {option_val}...")
                await self.api.update(project_key, type_key, issue_id, [{
                    "field_key": field_key,
