...
@pytest.fixture
def mock_metadata():
    with patch("src.providers.project.work_item_provider.MetadataService") as mock_cls:
...
async def test_create_issue(mock_api, mock_metadata):
    ...
    assert any(f["field_key"] == "field_description" and f["field_value"] == "Desc" for f in fields)
    assert any(f["field_key"] == "owner" and f["field_value"] == "user_456" for f in fields)
